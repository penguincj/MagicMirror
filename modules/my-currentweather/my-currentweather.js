...
Module.register("my-currentweather",{
	...
	getDom: function() {
        Log.info('my_current_weather  getDom');
	...
	notificationReceived: function(notification, payload, sender) {
		...
		if (notification == "SHOW_MY_CURRENTWEATHER") {
            if (payload == true ) {
                Log.info('my_current_weather received show true');
	...
	processWeather: function(data) {
        ...
        var weather_data = data.HeWeather6[0];
		...
		if (this.config.useBeaufort){
			this.windSpeed = this.ms2Beaufort(this.roundValue(this.now.wind_spd));
		} else if (this.config.useKMPHwind) {
			this.windSpeed = parseFloat((this.now.wind_spd * 60 * 60) / 1000).toFixed(0);
		} else {
			this.windSpeed = parseFloat(this.now.wind_spd).toFixed(0);
		}

		// ONLY WORKS IF TEMP IN C //
		var windInMph = parseFloat(this.now.wind_spd * 2.23694);
		...
		this.windDirection = this.deg2Cardinal(this.now.wind_dir);
		this.windDeg = this.now.wind_deg;
		this.weatherType = this.config.iconTable[this.now.cond_code];
		this.weather = this.now.cond_txt;
