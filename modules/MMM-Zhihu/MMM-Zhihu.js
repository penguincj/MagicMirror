...
Module.register("MMM-Zhihu", {
	...
	start: function () {
        ...
        if (this.config.imagePaths.length == 0) {
            this.errorMessage = "MMM-Zhihu: Missing required parameter."
        }
        else {
        ...
        //setTimeout(function(){this.showCompliments();}, 5000);
        
        // ÓÃ·¨
        //this.sleep(3000);
        this.sleep(3000).then(() => {
            Log.info('in sleep show ');
            this.showCompliments(true);
            this.sleep(3000).then(() => {
                Log.info('in sleep2 hide');
                this.showCompliments(false);
            })
        })

		Log.info('after sleep');
		Log.info('before 2 sleep');
        sleep.sleep(5);
		Log.info('fater 2 sleep');
	...
	showCompliments: function(flag) {
        this.sendNotification('SHOW_COMPLIMENTS', flag);
    },

    /*
	showCompliments: function() {
        this.sendNotification('SHOW_COMPLIMENTS', 0);
    },
    */
